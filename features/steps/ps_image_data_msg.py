...
try:
    ...
    from polysync.data_model._internal.compare import ps_image_data_msg_type_convert_testable, Py_ps_image_data_msg_initialize_random
    from polysync.data_model.message_support.ps_image_data_msg import publish, subscribe
except ImportError:
    raise ImportError(
        'Py_ps_image_data_msg module dependencies \
...
@when('I convert the ps_image_data_msg back to a Py_ps_image_data_msg')
def step_impl(context):
    pass

@then('the ps_image_data_msg values are equivalent to each Py_ps_image_data_msg value')
def step_impl(context):
    msg = Py_ps_image_data_msg_initialize_random()
    result = ps_image_data_msg_type_convert_testable(msg)
...
@then('a {exeption} indicates the type was not Py_ps_image_data_msg')
...
def Py_ps_image_data_msg_handler(msg):
    if msg.header.src_guid == GLOBAL_GUID:
        global GLOBAL_TIMESTAMP
        GLOBAL_TIMESTAMP = msg.header.timestamp

@given(u'I have a licensed PsNode for publishing Py_ps_image_data_msg')
def step_impl(context):
    assert context.node_ref
    global GLOBAL_GUID
    GLOBAL_GUID = context.my_guid

@given(u'I have a Py_ps_image_data_msg')
def step_impl(context):
    context.msg = Py_ps_image_data_msg()
    context.msg.header.timestamp = 0xFFFF

@given(u'I have a handler for Py_ps_image_data_msg subscription')
def step_impl(context):
    assert Py_ps_image_data_msg_handler
    subscribe(handler=Py_ps_image_data_msg_handler)

@when(u'I publish my Py_ps_image_data_msg')
def step_impl(context):
    publish(context.msg)

@then(u'I receive the corresponding Py_ps_image_data_msg in my handler')
def step_impl(context):
    global GLOBAL_TIMESTAMP
